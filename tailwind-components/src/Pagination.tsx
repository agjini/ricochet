...
export function Pagination({
  ...
  return (
    <div
      className={`bg-white px-4 py-3 flex items-center justify-between sm:px-6 ${className}`}
    >
      <div
        className={`flex-1 flex justify-between sm:hidden items-center ${
      ...
      <div
        className={`hidden sm:flex-1 sm:flex sm:items-center ${
          withoutDetails ? "sm:justify-end" : "sm:justify-between"
        }`}
      >
        {!withoutDetails && (
          <div>
            <p className="text-sm text-gray-700">
              {message
                ? message(page + 1, total)
                : `${page * perPage + 1} à ${Math.min(
                    total,
                    (page + 1) * perPage
                  )} sur ${total}`}
            </p>
        ...
        <div>
          ...
          >
            ...
            {pages.map((p, i) => (
              <a
                key={`page_${i}`}
                className={`cursor-pointer relative min-w-pageButton inline-flex items-center justify-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium hover:bg-gray-50 ${
                  page === p
